...
describe('/threads/{threadId}/comments/{commentId}/replies endpoint', () => {
  ...
  describe('when POST /threads/{threadId}/comments/{commentId}/replies', () => {
    it('should response 401 when missing authenticaion', async () => {
      ...
      const response = await server.inject({
        method: 'POST',
        url: '/threads/thread-test-999/comments/comment-test-999/replies',
      });
    ...
    it('should response 201 and persisted reply', async () => {
      ...
      const server = await createServer(container);
      const {accessToken} = await ServerTestHelper
          .getAccessTokenUserIdHelper({server, username: 'x'});
  ...
  describe('when DELETE /threads/{threadId}/comments/{commentId}/replies/{replyId}', () => {
    it('should response 401 when missing authenticaion', async () => {
      ...
      const response = await server.inject({
        method: 'DELETE',
        url: '/threads/thread-test-999/comments/comment-test-999/replies/reply-test-999',
      });
    ...
    it('should response 403 when user is not the owner', async () => {
      await UsersTableTestHelper.addUser({username: 'user-124'});
      await ThreadsTableTestHelper.addThread({});
      await CommentsTableTestHelper.addComment({});
